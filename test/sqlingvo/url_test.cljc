...
(def urls
  ...
  (gen/elements
   ["mysql://localhost/sqlingvo"
    "postgresql://tiger:scotch@localhost:5432/sqlingvo?a=1&b=2"]))
...
(deftest test-parse
  (let [db (url/parse "mysql://localhost:5432/sqlingvo")]
    (is (nil? (:pool db)))
...
(deftest test-parse-with-query-params
  (let [db (url/parse "postgresql://tiger:scotch@localhost:5432/sqlingvo?a=1&b=2")]
    (is (nil? (:pool db)))
...
(deftest test-format-url
  (let [url "postgresql://tiger:scotch@localhost/sqlingvo?a=1&b=2"]
    (is (= (url/format (url/parse url)) url)))
  (let [url "postgresql://tiger:scotch@localhost:5432/sqlingvo?a=1&b=2"]
    (is (= (url/format (url/parse "postgresql://tiger:scotch@localhost:5432/sqlingvo?a=1&b=2"))
           "postgresql://tiger:scotch@localhost/sqlingvo?a=1&b=2"))))
